...
class GeminiQuantixOpenEnded(BaseModel):
    """Pydantic schema for open-ended qualitative responses generated by Gemini 2.5."""

    convincing_element: str
    hesitation_trigger: str
    aha_moment: str
    most_frustrating_moment: str
    missing_feature_expected: str
    nps_reason: str
    magic_wand_change: str
...
def generate_quantix_feedback(
    ...
    # 7. Attempt Gemini 2.5 open-ended answers synthesis
    use_gemini = os.getenv("SIMULACRA_USE_GEMINI", "true").lower() in ("true", "1")
    if use_gemini:
        client = get_gemini_client()
        if client:
            try:
                llm_open_ended = _generate_gemini_quantix_open_ended(
                    persona=persona,
                    metrics=metrics,
                    telemetry=telemetry,
                    client=client,
                ...
                if llm_open_ended:
                    convincing_element = llm_open_ended.convincing_element
                    hesitation = llm_open_ended.hesitation_trigger
                    aha = llm_open_ended.aha_moment
                    most_frustrating = llm_open_ended.most_frustrating_moment
                    missing_feature = llm_open_ended.missing_feature_expected
                    nps_reason = llm_open_ended.nps_reason
                    magic_wand = llm_open_ended.magic_wand_change
                    pitch = llm_open_ended.one_sentence_pitch
                    transformation = llm_open_ended.expected_transformation
            except Exception as exc:
                logger.warning(
                    f"Gemini 2.5 Quantix survey synthesis failed: {exc}. Using heuristic responses."
                )
...
def _generate_gemini_quantix_open_ended(
    persona: Persona,
    metrics: SessionMetrics,
    telemetry: list[TelemetryEvent],
    client: Any,
) -> GeminiQuantixOpenEnded | None:
    """Use Gemini 2.5 to generate personalized open-ended answers for Quantix survey."""
    ...
    prompt = f"""
You are {persona.name} ({persona.role}, age {persona.age}) completing an in-depth product feedback survey for Quantix (a quantitative risk management platform).
Answer these qualitative questions authentically in the first person based on your background ({persona.technical_skill.value}), biases ({', '.join(persona.biases)}), and actual test experience:

SESSION TELEMETRY:
- Goal: {persona.primary_goal}
- Duration: {metrics.duration_seconds}s, Total Steps: {metrics.total_steps}
- Task Success: {metrics.task_success}, Exit Reason: {metrics.exit_reason.value}
- Rage Clicks: {metrics.rage_clicks_total}, Friction Score: {metrics.friction_score:.1f}/100

JOURNEY EVENTS:
{chr(10).join(events_summary) if events_summary else "  (No interaction events)"}

Fill out these 9 survey fields in your own genuine voice:
1. 'convincing_element': What specific heading, stat, or feature convinced you to explore further?
2. 'hesitation_trigger': What caused hesitation, doubt, or friction on the landing page?
3. 'aha_moment': When did the product value click, or what made you see the concept clearly?
4. 'most_frustrating_moment': The most frustrating, confusing, or broken moment during your session.
5. 'missing_feature_expected': A specific feature or capability you expected but couldn't find.
6. 'nps_reason': The key reason for your likelihood to recommend rating.
7. 'magic_wand_change': If you could change one thing about Quantix with a magic wand, what would it be?
8. 'one_sentence_pitch': How would you explain what Quantix is to a colleague in your own words?
9. 'expected_transformation': What high-level business or personal transformation would you expect if using Quantix?
""".strip()
    ...
    resp = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            ...
            response_schema=GeminiQuantixOpenEnded,
            temperature=0.3,
        ),
    )
    ...
    return GeminiQuantixOpenEnded.model_validate_json(resp.text)
